...
nb_label = 7  # number of classes (e.g., 4)
imageSize = 224
lr_d = 0.0002
lr = 0.0002
...
def train_acgan(discriminator, generator, train_loader, num_epochs, metrics, batch_size=32):
    ...
    total_feat_mean_real = {i: torch.zeros(ndf, device=device) for i in range(nb_label)}
...
if __name__=='__main__':
    ...
    generator = netG(nz, ngf, 3).to(device)
    discriminator = netD(ndf, 3, 7).to(device)
    
    train_loader, val_dl = load_train_test_df()
    
    train_subset = torch.utils.data.Subset(train_loader.dataset, np.random.choice(len(train_loader.dataset), 1000, replace=False))
    subset_loader = torch.utils.data.DataLoader(train_subset, batch_size=batch_size, shuffle=True)
    
    generator.train()
    discriminator.train()
    
    train_acgan(discriminator, generator, subset_loader, num_epochs, metrics)
